...
class Feed_Converter {

	function __construct() {

		add_filter( 'the_content', array( $this, 'change_images' ), 20 );

	}

	public function change_images( $content ) {
		if ( is_feed() ) {
			...
			/* Gary: I cannot figure a way to combine/simplify the captions--getElementsByTagName
			 * only accepts one thing. Using * doesn't seem correct either, as it would target
			 * anything with a style. I don't know what other elements might have an inline style,
			 * but seems arbitary to decide they ALL must go. */
			...
			foreach ( $images as $image ) {
				 ...
				 */
					...
					if ( ( strpos( $class, 'alignright' ) !== false ) && ( $width < $maxwidth ) ) {
					...
					elseif ( ( strpos( $class, 'alignleft' ) !== false ) && ( $width < $maxwidth ) ) {
			...
			// Strips extra added by new DOMDocument
	 ...
	 */
	private function get_image_id( $attachment_url = '' ) {
